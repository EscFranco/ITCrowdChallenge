    ...
    useEffect(() => {
        if (categoryName === undefined) {
            fetch("http://localhost:8080/productos", {
                ...
                .then((res) => res.json())
                .then(json => {
                    const list = json.filter((element) => element.important === "true")
                    setProductos(list)
                })
        } else {
    ...
    return (
        <main>
            ...
            <div className="bg-white">
                <div className="max-w-2xl mx-auto py-4 px-4 sm:py-8 sm:px-6 lg:max-w-full lg:px-8">
                    {categoryName === undefined ? (
                        <h2 className="text-2xl font-extrabold tracking-tight text-gray-900 pb-3 text-center">PRODUCTOS DESTACADOS</h2>
                    ) :
                        (
                            <h2 className="text-2xl font-extrabold tracking-tight text-gray-900 pb-3 text-center uppercase">{categoryName.replace(/-/g, " ")}</h2>
                        )
                    }
